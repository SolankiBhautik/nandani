...
const Navbar = () => {
  ...
  const getLinkClass = ({ isActive }: { isActive: boolean }) =>
    `text-sm md:text-lg font-semibold transition-colors ${isActive ? "text-primary font-medium" : "text-black hover:text-primary"
    }`;

  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 md:w-[95%] max-w-6xl">
      <div className="bg-white/40 backdrop-blur-xl rounded-full px-4 py-3 shadow-lg border-white border-2 w-fit md:w-auto mx-auto">
